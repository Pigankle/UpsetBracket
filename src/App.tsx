...
function matchupsToSavedBracket(
  matchups: Matchup[],
  name: string,
): SavedBracket {
  ...
  matchups.forEach((m) => {
    if (m.gameCode && m.winner)
      picks[m.gameCode] =
        m.winner === "top" ? m.topTeam.name : m.bottomTeam.name;
  });
  return { name, picks };
}
...
export default function App() {
  ...
  const [viewMatchups, setViewMatchups] = useState<Matchup[]>(() =>
    createInitialBracket(tournamentData),
  );
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      if (resultsData.data) {
        const adapted: Record<string, Record<string, unknown>> = {};
        (resultsData.data as MarchMadnessGame[]).forEach((g) => {
          adapted[g.game_code] = adaptGame(g);
        });
        setGames(adapted);
      }
  ...
  const handleSaveBracket = async () => {
    ...
    setSaveStatus("idle");
    const personName =
      profile.is_admin && onBehalfOf.trim()
        ? onBehalfOf.trim()
        ...
        <div
          ...
          <div style={{ width: "100%", overflowX: "auto" }}>
            <BracketDisplay
              ...
              onUpdateBracket={() => {}}
              bracketName={viewingBracketName}
              onUpdateBracketName={() => {}}
  ...
  const Nav = () => (
    <div
      ...
      <span style={{ fontWeight: 700, fontSize: 15, marginRight: 16 }}>
        🏀 2026 Bracket Pool
      </span>
      <NavBtn
        label="My Brackets"
      ...
      <NavBtn
        label="Leaderboard"
        active={view === "leaderboard" || view === "view-bracket"}
        onClick={() => setView("leaderboard")}
      />
      {profile.is_admin && (
        <NavBtn
          ...
          <button
            onClick={() =>
              printBracket(
                ...
                viewingBracketName,
                "",
                calculateScore(viewMatchups, games),
                viewingTiebreaker,
              )
            }
            style={{
          ...
          <>
            ...
            <button
              ...
              style={{
                ...
                color: "#fff",
                border: "none",
                cursor: saving || !bracketName.trim() ? "default" : "pointer",
                fontSize: 13,
                opacity: saving || !bracketName.trim() ? 0.6 : 1,
              }}>
              {saving ? "Saving..." : bracketId ? "Update" : "Save Bracket"}
            </button>
            {saveStatus === "saved" && (
              <span style={{ color: "#2a7a2a", fontSize: 13 }}>✓ Saved</span>
            )}
            {saveStatus === "error" && (
              <span style={{ color: "#b00000", fontSize: 13 }}>
                Save failed
              </span>
            )}
            {!bracketName.trim() && (
              <span style={{ fontSize: 12, color: "#b00000" }}>
                Enter a bracket name to save
              </span>
            )}
          </>
        ) : (
          <span style={{ fontSize: 13, color: C.seed }}>
            🔒 Submissions closed — {deadlineLabel}
          </span>
        )}
      </div>
      <div style={{ width: "100%", overflowX: "auto" }}>
