...
const roundLayout = (roundIdx: number) => {
  const count = 8 >> roundIdx;
  const stride = SLOT * (1 << roundIdx);
  return { count, stride };
};
...
function FinalFourCenter({ matchups, games, onPick, bracketName, onUpdateBracketName, totalScore, useTestData, tiebreakerScore, onUpdateTiebreakerScore, readOnly }: FinalFourCenterProps) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minWidth: 280, padding: '0 8px', gap: 0 }}>
      <input
        value={bracketName}
      ...
      <div style={{ marginTop: 16, width: '100%', fontSize: 11, color: C.text, borderTop: `1px solid ${C.divider}`, paddingTop: 10 }}>
        ...
        <div style={{ fontSize: 11, fontWeight: 700, color: C.points, marginTop: 6 }}>Total: {totalScore} pts</div>
      </div>
    </div>
  );
}
...
export default function BracketDisplay({
  initialMatchups, onUpdateBracket, bracketName, onUpdateBracketName,
  totalScore, useTestData, onUpdateUseTestData, games,
  tiebreakerScore, onUpdateTiebreakerScore, readOnly,
}: BracketDisplayProps) {
  ...
  const handlePick = (gameCode: string, position: 'top' | 'bottom') => {
    ...
    const clearPath = (code: string) => {
      ...
      if (isOrigin) {
        if (pos === 'top') nm.topTeam = selected; else nm.bottomTeam = selected;
      } else {
        if (pos === 'top') {
